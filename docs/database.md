...
Streamlit Cloud by setting `DB_PROVIDER="mongodb"` and `APP_MONGO_URI`.
...
Search can be used for Story Memory by setting `VECTOR_PROVIDER=mongodb_vector`;
local Chroma remains available with `VECTOR_PROVIDER=chroma`.

Primary app persistence and backup sync use separate MongoDB settings:
- `APP_MONGO_URI` / `APP_MONGO_DATABASE` for live app collections.
- `BACKUP_MONGO_URI` / `BACKUP_MONGO_DATABASE` for snapshot backup documents.
