...
- **Saved Defaults**: The last selected provider/model is saved to the `[llm]` section in `.streamlit/secrets.toml` for local and Streamlit Cloud use; containers can use `DEFAULT_LLM_PROVIDER` and `DEFAULT_LLM_MODEL`
